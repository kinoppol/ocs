<?php
        ...
        foreach($trainerDevData as $k=>$v){
            ...
            $tableRows[]=array(
                'no'=>$i,
                'trainerDev_duration'=>dateThai($v->start_date,true,false,true).'-'.dateThai($v->end_date,true,false,true),
                'trainerDev_plcae'=>$v->dev_place,
                'trainerDev_subject'=>$v->subject,
                'manage'=>'
                <a href="'.site_url('public/gov/trainerDevDetail/'.$v->id).'" class="btn btn-xs btn-warning waves-effect"><i class="material-icons">edit</i>แก้ไข</a>
                <a href="'.site_url('public/gov/trainerDevDelete/'.$v->id).'" class="btn btn-xs btn-danger" onclick="return confirm(\'ยืนยันการลบข้อมูล\');"><i class="material-icons">delete</i> ลบ</a>',
            );
        }
