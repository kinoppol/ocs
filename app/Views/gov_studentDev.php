<?php
        ...
        foreach($studentDevData as $k=>$v){
            ...
            $tableRows[]=array(
                'no'=>$i,
                'studentDev_duration'=>dateThai($v->start_date,true,false,true).'-'.dateThai($v->end_date,true,false,true),
                'studentDev_plcae'=>$v->dev_place,
                'studentDev_subject'=>$v->subject,
                'manage'=>'
                <a href="'.site_url('public/gov/studentDevDetail/'.$v->id).'" class="btn btn-xs btn-warning waves-effect"><i class="material-icons">edit</i>แก้ไข</a>
                <a href="'.site_url('public/gov/studentDevDelete/'.$v->id).'" class="btn btn-xs btn-danger" onclick="return confirm(\'ยืนยันการลบข้อมูล\');"><i class="material-icons">delete</i> ลบ</a>',
            );
        }
